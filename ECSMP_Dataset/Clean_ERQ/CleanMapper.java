...
public class CleanMapper extends Mapper<LongWritable, Text, Text, Text> { // Clean ERQ_Table
    @Override
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
      ...
      if(key.get() == 0) { // if is header
          context.write(new Text("ID,Complete,Expressive_Suppression,Cognitive_Reappraisal"), new Text("")); // new header
      } else if(line.length == 17) {
          String userId = line[0];
          String complete = line[3];
          String esScore = line[15];
          String crScore = line[16];
          if(isInt(line[0])) { // if is user ID 
            context.write(new Text(userId + "," + complete + "," + esScore + "," + crScore), new Text(""));
          } 
      }
    }
